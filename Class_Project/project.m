...
% Even thoug we know there is a trend, we fit original model
nstep =1;
[forecast_data,forecast_residuals]=forecast_nstep(Model,y,full_data,nstep);
figure()
plot(forecast_data)
hold on
plot(full_data(train_size+1:end))
...
forecastRSS=sum(forecast_residuals.^2);
% 
...
yF = forecast(Model,y,31);
MSE=27.09;
[var,std_nstep]=variance_nstep(G,MSE);

UB=yF + 1.96*std_nstep';

LB=yF - 1.96*std_nstep';
figure()
plot(yF)
hold on

plot(full_data(train_size+1:train_size+31))
